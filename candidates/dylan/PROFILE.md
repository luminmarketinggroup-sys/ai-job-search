...
## Output Paths (dual-candidate repo)
- Profile: `candidates/dylan/PROFILE.md` + `candidates/dylan/profile/`
- CVs: `candidates/dylan/cv/main_<company>.tex` (master: `main_example.tex`; local: `main_local_edmonton.tex`)
- Cover letters: `candidates/dylan/cover_letters/cover_<company>_<role>.tex`
- Tracker: `candidates/dylan/job_search_tracker.csv`
- Shared LaTeX cover class: symlinked from repo-root `cover_letters/`

## Workflow for New Job Applications
1. Confirm candidate is **Dylan** (not Lillian)
2. Job posting from scrape, URL, or text
3. Evaluate fit (skills, experience, culture, location). **Dual-track:** local survival near 11110 68 Ave NW may auto-draft without ask; career roles auto-draft when Good/High (≥60) without ask (user-approved). Still show a brief score in the PR/run log.
4. Draft targeted CV and cover letter under `candidates/dylan/`
5. **Verify both documents** (see root `CLAUDE.md` Verification Checklist)
6. Update `candidates/dylan/job_search_tracker.csv` + PR; prepare interview talking points when moving to interview stage
