# Daily job search — LAST RUN

**Date:** 2026-08-23  
**Branch push target:** `cursor/dylan-michael-profile-setup-7ff7` (PR #1)  
**Resend:** unset (mailto / portal only)

## New local drafts (5)
| Company | Role | Link | Files |
|---------|------|------|-------|
| Hercules Crane & Lifting | Shipper/Receiver | https://ca.linkedin.com/jobs/view/4442172759 | `cv/main_local_edmonton.pdf` · `APPLICATIONS_LOCAL.md#EF` (prior email #AJ — do not re-send) |
| Home Depot South Common | Order Picker FT | https://ca.linkedin.com/jobs/view/4456800500 | `cv/main_local_edmonton.pdf` · `APPLICATIONS_LOCAL.md#EG` |
| Home Depot South Common | Overnight Freight FT | https://ca.linkedin.com/jobs/view/4456683545 | `cv/main_local_edmonton.pdf` · `APPLICATIONS_LOCAL.md#EH` |
| Auto Parts Authority | Auto Parts Counter Sales | https://ca.linkedin.com/jobs/view/4453254507 | `cv/main_local_edmonton.pdf` · `APPLICATIONS_LOCAL.md#EI` |
| RONA Leduc | Yard Associate | https://ca.linkedin.com/jobs/view/4457863307 | `cv/main_local_edmonton.pdf` · `APPLICATIONS_LOCAL.md#EJ` |

## New career drafts (2)
| Company | Role | Fit | Link | Files |
|---------|------|-----|------|-------|
| Propellic | Paid Media Manager | ~72 | https://ca.linkedin.com/jobs/view/4455480764 | `cv/main_propellic.pdf` · `cover_letters/cover_propellic_paid_media_manager.pdf` |
| Jobgether (partner) | SEO Strategist | ~61 | https://ca.linkedin.com/jobs/view/4456255158 | `cv/main_jobgether_seo_strategist.pdf` · `cover_letters/cover_jobgether_seo_strategist.pdf` |

## Summary
- **5** new local drafts  
- **2** new career drafts  
- Checklists: `APPLICATIONS_LOCAL.md`, `APPLICATIONS_READY.md`
