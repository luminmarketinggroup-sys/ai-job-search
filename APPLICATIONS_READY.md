...
**Mailto this run:** Argus General Labour Nisku + Edmonton Saw Operator → `mailto:info@argusinnovates.com` with `cv/main_local_edmonton.pdf` (Resend attempted if key maps to `RESEND_API_KEY`). No verified public apply email for Power Digital or Marble — use Greenhouse / Lever (+ Plum survey). Fountain Tire Roper via careers / in-store.
